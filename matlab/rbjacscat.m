...
for i=1:length(wv)
    Jscat.scatamp(:,(i*size(Jd,2)+1):(i+1)*size(Jd,2)) =...
        rbjacscatamp(Jd(wv{i}), dcoeff, str2double(wv{i}), scatpow);
    Jscat.scatpow(:,(i*size(Jd,2)+1):(i+1)*size(Jd,2)) =...
        rbjacscatpow(Jd(wv{i}), dcoeff, str2double(wv{i}));
end
